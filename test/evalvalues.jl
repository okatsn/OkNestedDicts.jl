...
@testset "@evalvalues tests" begin
    ...
    @testset "DateTime and DatePeriod evaluation" begin
        ...
        # Test some edge cases and error handling for dates
        @test_throws Exception @evalvalues Dict("bad" => "DateTime(2022, 13, 1)")  # Invalid month
        @test_throws Exception @evalvalues Dict("bad" => "Date(2022, 2, 30)")      # Invalid day
        @test_throws Exception @evalvalues Dict("bad" => "Time(25, 0, 0)")         # Invalid hour

        # Test date formatting and parsing
        @test (@evalvalues Dict("formatted" => "Dates.format(DateTime(2022, 1, 1), \"yyyy-mm-dd\")"))["formatted"] == "2022-01-01"
        @test (@evalvalues Dict("parsed" => "DateTime(\"2022-01-01T12:00:00\")"))["parsed"] == DateTime(2022, 1, 1, 12, 0, 0)

        # Test some useful date calculations
        @test (@evalvalues Dict("last" => "Dates.lastdayofmonth(Date(2022, 2, 1))"))["last"] == Date(2022, 2, 28)
        @test (@evalvalues Dict("first" => "Dates.firstdayofweek(Date(2022, 1, 5))"))["first"] == Date(2022, 1, 3)  # Monday of that week
        @test (@evalvalues Dict("day" => "Dates.dayofyear(Date(2022, 1, 1))"))["day"] == 1
    end
end
